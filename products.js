...
function sizeSelectorMarkup(p){if(!isRingCategory(p))return '';if(!productHasSizes(p))return `<div class="size-selector size-selector-empty" data-size-selector="${p.id}"><div class="size-selector-head"><span>Size:</span><small>قياسات الخاتم</small></div><div class="size-empty-note">لا توجد قياسات مضافة لهذا الخاتم بعد</div></div>`;return `<div class="size-selector" data-size-selector="${p.id}"><div class="size-selector-head"><span>Size:</span><small>اختر القياس</small></div><div class="size-options">${p.sizes.map(x=>{const av=getVisibleSizeStock(p,x.size)>0;return `<button type="button" class="size-option ${av?'available':'unavailable'}" data-size-product="${p.id}" data-size-value="${escapeHtml(x.size)}" ${av?'':'disabled'}>Size: ${escapeHtml(x.size)}</button>`}).join('')}</div></div>`}
...
function openImageLightbox(){const imgs=window.__detailImages||[];if(!imgs.length)return;let box=document.getElementById('fiddaImageLightbox');if(!box){box=document.createElement('div');box.id='fiddaImageLightbox';box.className='fidda-lightbox hidden';box.innerHTML='<div class="fidda-lightbox-backdrop" data-lightbox-close></div><button class="fidda-lightbox-close" type="button" data-lightbox-close aria-label="إغلاق">×</button><button class="fidda-lightbox-nav prev" type="button" onclick="changeLightboxImage(-1)" aria-label="الصورة السابقة">‹</button><img id="fiddaLightboxImage" alt=""><button class="fidda-lightbox-nav next" type="button" onclick="changeLightboxImage(1)" aria-label="الصورة التالية">›</button><div class="fidda-lightbox-count" id="fiddaLightboxCount"></div>';document.body.appendChild(box);box.querySelectorAll('[data-lightbox-close]').forEach(x=>x.addEventListener('click',closeImageLightbox));}box.classList.remove('hidden');document.body.classList.add('lightbox-open');updateLightboxImage()}
