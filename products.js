...
function sizeSelectorMarkup(p){if(!productHasSizes(p))return '';return `<div class="size-selector" data-size-selector="${p.id}"><div class="size-selector-head"><span>Size:</span><small>اختر القياس</small></div><div class="size-options">${p.sizes.map(x=>{const av=getVisibleSizeStock(p,x.size)>0;return `<button type="button" class="size-option ${av?'available':'unavailable'}" data-size-product="${p.id}" data-size-value="${escapeHtml(x.size)}" ${av?'':'disabled'}>Size: ${escapeHtml(x.size)}</button>`}).join('')}</div></div>`}
