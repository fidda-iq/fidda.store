...
function normalizeProduct(p){
  ...
  return {...p,code:String(p?.code||p?.product_code||''),sort_order:Number.isFinite(Number(p?.sort_order))?Number(p.sort_order):0,category_sort_order:Number.isFinite(Number(p?.category_sort_order))?Number(p.category_sort_order):Number.isFinite(Number(p?.sort_order))?Number(p.sort_order):0,featured_sort_order:Number.isFinite(Number(p?.featured_sort_order))?Number(p.featured_sort_order):Number.isFinite(Number(p?.sort_order))?Number(p.sort_order):0,stock,sizes,images:(Array.isArray(p?.images)&&p.images.length?p.images:[p?.image||'']).filter(Boolean),material:p?.material||'فضة',payment:p?.payment||'الدفع عند الاستلام',customFields:Array.isArray(p?.customFields)?p.customFields.filter(x=>x&&String(x.label||'').trim()&&String(x.value||'').trim()).map(x=>({label:String(x.label).trim(),value:String(x.value).trim()})):[]};
}
...
function renderRelatedProducts(currentId){const el=document.getElementById('relatedProducts');const section=document.getElementById('relatedProductsSection');if(!el)return;const current=getProducts().find(p=>Number(p.id)===Number(currentId));if(!current){if(section)section.classList.add('hidden');return}const all=getProducts().filter(p=>Number(p.id)!==Number(currentId));const sameCategory=all.filter(p=>p.category===current.category);const others=all.filter(p=>p.category!==current.category);sameCategory.sort((a,b)=>(Number(a.category_sort_order)||0)-(Number(b.category_sort_order)||0)||Number(a.id)-Number(b.id));others.sort((a,b)=>(Number(a.sort_order)||0)-(Number(b.sort_order)||0)||Number(a.id)-Number(b.id));const list=[...sameCategory,...others].slice(0,4);if(!list.length){el.innerHTML='';if(section)section.classList.add('hidden');return}if(section)section.classList.remove('hidden');el.innerHTML=list.map(productCard).join('');initReveal();updateAllCustomerStockUI()}
...
function openImageLightbox(){const imgs=window.__detailImages||[];if(!imgs.length)return;let box=document.getElementById('fiddaImageLightbox');if(!box){box=document.createElement('div');box.id='fiddaImageLightbox';box.className='fidda-lightbox hidden';box.innerHTML='<div class="fidda-lightbox-backdrop" data-lightbox-close></div><button class="fidda-lightbox-close" type="button" data-lightbox-close aria-label="العودة إلى المنتج">×</button><div class="fidda-zoom-wrap"><img id="fiddaLightboxImage" alt=""></div><div class="fidda-lightbox-count" id="fiddaLightboxCount"></div><div class="fidda-zoom-controls"><button type="button" data-zoom="out" aria-label="تصغير">−</button><button type="button" data-zoom="reset" aria-label="إرجاع الحجم الطبيعي">1:1</button><button type="button" data-zoom="in" aria-label="تكبير">+</button></div>';document.body.appendChild(box);box.querySelectorAll('[data-lightbox-close]').forEach(x=>x.addEventListener('click',closeImageLightbox));box.querySelectorAll('[data-zoom]').forEach(x=>x.addEventListener('click',()=>{const action=x.dataset.zoom;if(action==='in')setLightboxZoom((window.__lightboxZoom||1)+.25);else if(action==='out')setLightboxZoom((window.__lightboxZoom||1)-.25);else setLightboxZoom(1)}));initLightboxZoom(box)}if(box.classList.contains('hidden')){box.classList.remove('hidden');document.body.classList.add('lightbox-open');window.__lightboxZoom=1;if(!history.state?.fiddaLightbox){history.pushState({fiddaLightbox:true},'',location.href)}updateLightboxImage();return}updateLightboxImage()}
function closeImageLightbox(fromPopState=false){const box=document.getElementById('fiddaImageLightbox');if(!box||box.classList.contains('hidden'))return;if(!fromPopState&&history.state?.fiddaLightbox){history.back();return}box.classList.add('hidden');document.body.classList.remove('lightbox-open');setLightboxZoom(1)}
function updateLightboxImage(){const box=document.getElementById('fiddaImageLightbox'),imgs=window.__detailImages||[],i=Number(window.__detailIndex)||0;if(!box||!imgs.length)return;const img=box.querySelector('#fiddaLightboxImage');if(img){img.classList.add('image-loading');const src=fiddaFastImageUrl(imgs[i]||'',1800,88);const reveal=()=>{if(Number(window.__detailIndex)!==i)return;img.src=src;img.classList.remove('image-loading');setLightboxZoom(1)};const cached=window.__detailImageCache?.[i];if(cached?.complete&&cached.naturalWidth){reveal()}else{const loader=cached||new Image();loader.decoding='async';loader.onload=reveal;loader.onerror=()=>img.classList.remove('image-loading');if(!cached)loader.src=src}}const c=box.querySelector('#fiddaLightboxCount');if(c)c.textContent=`${i+1} / ${imgs.length}`}
function setLightboxZoom(value){const box=document.getElementById('fiddaImageLightbox'),img=box?.querySelector('#fiddaLightboxImage');if(!img)return;const z=Math.min(4,Math.max(1,Number(value)||1));window.__lightboxZoom=z;img.style.transform=`scale(${z})`;img.classList.toggle('zoomed',z>1);if(z<=1){img.style.left='0px';img.style.top='0px';window.__lightboxPanX=0;window.__lightboxPanY=0}}
function initLightboxZoom(box){const img=box.querySelector('#fiddaLightboxImage');if(!img||img.dataset.zoomReady)return;img.dataset.zoomReady='1';let sx=0,sy=0,lastTap=0,pinchStart=0,baseZoom=1,panX=0,panY=0;img.addEventListener('wheel',e=>{e.preventDefault();setLightboxZoom((window.__lightboxZoom||1)+(e.deltaY<0?.2:-.2))},{passive:false});img.addEventListener('dblclick',e=>{e.preventDefault();setLightboxZoom((window.__lightboxZoom||1)>1?1:2)});img.addEventListener('pointerdown',e=>{sx=e.clientX;sy=e.clientY;panX=window.__lightboxPanX||0;panY=window.__lightboxPanY||0;img.setPointerCapture?.(e.pointerId)});img.addEventListener('pointermove',e=>{if((window.__lightboxZoom||1)<=1)return;img.style.left=`${panX+e.clientX-sx}px`;img.style.top=`${panY+e.clientY-sy}px`});img.addEventListener('pointerup',e=>{if((window.__lightboxZoom||1)<=1)return;window.__lightboxPanX=panX+e.clientX-sx;window.__lightboxPanY=panY+e.clientY-sy;const now=Date.now();if(now-lastTap<300){setLightboxZoom((window.__lightboxZoom||1)>1?1:2)}lastTap=now});img.addEventListener('pointercancel',()=>{});let p1=null,p2=null;box.addEventListener('pointerdown',e=>{if(e.pointerType==='touch'){if(!p1)p1=e;else if(!p2){p2=e;pinchStart=Math.hypot(p1.clientX-p2.clientX,p1.clientY-p2.clientY);baseZoom=window.__lightboxZoom||1}}});box.addEventListener('pointermove',e=>{if(e.pointerType==='touch'){if(p1&&e.pointerId===p1.pointerId)p1=e;else if(p2&&e.pointerId===p2.pointerId)p2=e;if(p1&&p2){const d=Math.hypot(p1.clientX-p2.clientX,p1.clientY-p2.clientY);setLightboxZoom(baseZoom*(d/pinchStart))}}});box.addEventListener('pointerup',e=>{if(p1&&e.pointerId===p1.pointerId)p1=null;if(p2&&e.pointerId===p2.pointerId)p2=null});box.addEventListener('pointercancel',()=>{p1=p2=null})}
...
function renderProductDetail(){
  ...
  window.__detailImages=imgs;window.__detailIndex=0;window.__detailProductId=id;window.__detailSelectedSize='';preloadDetailImages(imgs.map(im=>fiddaFastImageUrl(im,1200,82)));
  const galleryOpen=el.querySelector('.gallery-open');
  if(galleryOpen){let startX=0,startY=0,moved=false;galleryOpen.addEventListener('pointerdown',e=>{startX=e.clientX;startY=e.clientY;moved=false;galleryOpen.classList.remove('swipe-next','swipe-prev');galleryOpen.setPointerCapture?.(e.pointerId)});galleryOpen.addEventListener('pointerup',e=>{const dx=e.clientX-startX,dy=e.clientY-startY;if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy)){moved=true;galleryOpen.classList.add(dx<0?'swipe-next':'swipe-prev');if(dx<0){changeDetailImageToLast()}else{changeDetailImage(1)}setTimeout(()=>galleryOpen.classList.remove('swipe-next','swipe-prev'),320);return}if(moved)return;const r=galleryOpen.getBoundingClientRect(),x=e.clientX-r.left;if(x<r.width*.28)changeDetailImage(1);else if(x>r.width*.72)changeDetailImage(-1);else openImageLightbox()});galleryOpen.addEventListener('pointercancel',()=>{moved=true});galleryOpen.addEventListener('click',e=>e.preventDefault());}
  const lightbox=document.getElementById('fiddaImageLightbox');
  if(lightbox){let sx=0,sy=0;lightbox.addEventListener('pointerdown',e=>{sx=e.clientX;sy=e.clientY});lightbox.addEventListener('pointerup',e=>{const dx=e.clientX-sx,dy=e.clientY-sy;if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy)&&((window.__lightboxZoom||1)<=1))changeLightboxImage(dx<0?1:-1)})}
...
function preloadDetailImages(imgs){window.__detailImageCache={};(imgs||[]).forEach((src,i)=>{if(!src)return;const im=new Image();im.decoding='async';im.src=src;im.decode?.().catch(()=>{});window.__detailImageCache[i]=im})}
function showDetailImageWhenReady(index){const imgs=window.__detailImages||[],src=imgs[index]||'',img=document.getElementById('detailMainImage');if(!img||!src)return;img.classList.add('image-loading');const cached=window.__detailImageCache?.[index];const reveal=()=>{if(Number(window.__detailIndex)!==index)return;img.src=src;const done=()=>img.classList.remove('image-loading');if(img.decode)img.decode().then(done).catch(done);else done()};if(cached?.complete&&cached.naturalWidth){reveal();return}const loader=cached||new Image();loader.decoding='async';loader.onload=reveal;loader.onerror=()=>{if(Number(window.__detailIndex)===index)img.classList.remove('image-loading')};if(!cached)loader.src=src}
function setDetailImage(index){const imgs=window.__detailImages||[];if(!imgs.length)return;index=(index+imgs.length)%imgs.length;window.__detailIndex=index;showDetailImageWhenReady(index);document.querySelectorAll('.gallery-thumb').forEach(x=>x.classList.toggle('active',Number(x.dataset.index)===index));const pos=document.getElementById('galleryPosition');if(pos)pos.textContent=`${index+1} / ${imgs.length}`;updateLightboxImage()}
function changeDetailImage(delta){setDetailImage((window.__detailIndex||0)+delta)}
function changeDetailImageToLast(){const imgs=window.__detailImages||[];if(!imgs.length)return;setDetailImage(imgs.length-1)}
...
function updateDetailQuantity(id){if(!sameProductId(id,window.__detailProductId))return;const p=getProducts().find(x=>sameProductId(x.id,id)),e=document.getElementById('detailQty'),btn=document.getElementById('detailAddButton');if(!p)return;const selectedSize=String(window.__detailSelectedSize||'');const needsSize=productHasSizes(p)&&!selectedSize;const max=needsSize?0:getRemainingAddableStock(p,selectedSize);if(e){e.max=max;if(max>0)e.value=Math.min(max,Math.max(1,Number(e.value||1)));else e.value=1;}if(btn){btn.disabled=needsSize?false:max<=0;btn.innerHTML=needsSize?'اختر القياس أولاً':(max>0?bagIcon()+'<span>أضف إلى السلة</span>':'غير متوفر')}updateDetailTotal()}
...
function renderCart(){
  ...
  const rows=cart.map(i=>{const p=products.find(x=>sameProductId(x.id,i.id));if(!p)return'';const size=String(i.size||''),available=getRemainingAddableStock(p,size),key=encodeURIComponent(`${p.id}::${size}`);return `<div class="cart-row reveal" data-cart-key="${key}"><a class="cart-product-link" href="product.html?id=${encodeURIComponent(p.id)}"><img src="${escapeHtml(fiddaFastImageUrl(productImages(p)[0]||'',360,74))}" alt="${escapeHtml(p.name)}" loading="lazy" decoding="async"></a><div class="cart-item-info"><a class="cart-item-name" href="product.html?id=${encodeURIComponent(p.id)}"><h3>${escapeHtml(p.name)}</h3></a><div class="cart-item-meta">${size?`<span class="cart-item-size"><b>Size</b><em>${escapeHtml(size)}</em></span>`:''}<span class="cart-item-unit-price"><b>السعر</b><em>${formatPrice(p.price)}</em></span></div><small class="cart-remaining">المتاح${size?` من القياس ${escapeHtml(size)}`:''}: ${available} قطعة</small></div><div class="qty-control"><button class="cart-minus" data-cart-action="minus" data-id="${p.id}" data-size="${escapeHtml(size)}" type="button">−</button><input class="cart-qty" data-cart-action="input" data-id="${p.id}" data-size="${escapeHtml(size)}" type="text" inputmode="numeric" value="${i.qty}" min="1" max="${available+Number(i.qty||0)}"><button class="cart-plus" data-cart-action="plus" data-id="${p.id}" data-size="${escapeHtml(size)}" type="button" ${available<=0?'disabled':''}>+</button></div><div class="cart-total-box"><span>الإجمالي</span><strong class="cart-row-total">${formatPrice(p.price*i.qty)}</strong></div><button class="remove-btn" data-cart-action="remove" data-id="${p.id}" data-size="${escapeHtml(size)}" type="button" aria-label="إزالة ${escapeHtml(p.name)}">${trashIcon()}</button></div>`}).join('');
...
function closeOrderSuccessModal(m){if(!m)return;m.classList.add('hidden');m.classList.remove('is-open');document.body.classList.remove('fidda-success-modal-open');document.body.style.overflow='';}function initOrderSuccessModal(){const m=document.getElementById('orderSuccessModal');if(!m)return;m.querySelectorAll('[data-close-success]').forEach(el=>el.addEventListener('click',()=>closeOrderSuccessModal(m)));document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!m.classList.contains('hidden'))closeOrderSuccessModal(m)})}
...
window.addEventListener('popstate',e=>{const box=document.getElementById('fiddaImageLightbox');if(box&&!box.classList.contains('hidden')&&!e.state?.fiddaLightbox)closeImageLightbox(true)});
