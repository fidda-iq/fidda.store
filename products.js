...
function productCard(p){const img=productImages(p)[0]||'',out=getRemainingAddableStock(p)<=0;return `<article class="product-card reveal ${out?'out-of-stock':''}"><a href="product.html?id=${encodeURIComponent(p.id)}" class="product-image"><img src="${escapeHtml(img)}" alt="${escapeHtml(p.name)}" loading="lazy"><span class="product-category">${escapeHtml(p.category)}</span>${productImages(p).length>1?`<span class="image-count">◈ ${productImages(p).length}</span>`:''}</a><div class="product-info"><a href="product.html?id=${encodeURIComponent(p.id)}"><h3>${escapeHtml(p.name)}</h3></a>${stockMarkup(p)}<div class="product-bottom"><strong>${formatPrice(p.price)}</strong><button class="add-btn" data-add-product="${p.id}" ${out?'disabled':''} onclick="event.preventDefault();addToCart(${Number(p.id)},1)">+</button></div></div></article>`}
