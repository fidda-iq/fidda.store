/* فِضّة FIDDA — متجر الزبائن | V82 secure order fix */
...
function updateCartSummary(){const t=cartTotals();const s=document.getElementById('cartSubtotal'),d=document.getElementById('cartDelivery'),total=document.getElementById('cartTotal');if(s)s.textContent=formatPrice(t.subtotal);if(d)d.textContent=formatPrice(t.delivery);if(total)total.textContent=formatPrice(Number(result.total ?? t.total))}
...
function setupCheckout(){
  ...
  f.addEventListener('submit',async e=>{
    ...
    try{
      ...
      localStorage.removeItem(CART_KEY);updateCartCount();f.classList.add('hidden');const success=document.getElementById('orderSuccess');success.classList.remove('hidden');success.innerHTML=`<div class="success-icon">✓</div><p class="eyebrow">ORDER CONFIRMED</p><h2>تم استلام طلبك بنجاح</h2><p>رقم الطلب: <b>${escapeHtml(result.id)}</b></p><p>الإجمالي: <b>${formatPrice(Number(result.total ?? t.total))}</b> شامل التوصيل.</p><p>سنتواصل معك لتأكيد الطلب وتجهيزه.</p>`;const modal=document.getElementById('orderSuccessModal');const details=document.getElementById('successDetails');if(modal&&details){details.className='success-details';details.innerHTML=`<div class="detail-row"><span>رقم الطلب</span><b>${escapeHtml(result.id)}</b></div><div class="detail-row"><span>الإجمالي</span><b>${formatPrice(Number(result.total ?? t.total))} شامل التوصيل</b></div>`;modal.classList.remove('hidden');document.body.style.overflow='hidden';}
    }catch(err){
