...
function stockMarkup(p){const visible=getCustomerVisibleStock(p);return `<span data-stock-product="${p.id}" class="stock-badge ${visible>0?'available':'unavailable'}"><i class="stock-dot"></i>${visible>0?`متوفر <b class="stock-number">${visible}</b> قطعة`:'غير متوفر'}</span>`}
function bagIcon(){return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5.5 8.5h13l-1 11H6.5l-1-11Z"></path><path d="M9 8.5a3 3 0 0 1 6 0"></path></svg>'}
function productCard(p){const img=productImages(p)[0]||'',out=getRemainingAddableStock(p)<=0;return `<article class="product-card reveal ${out?'out-of-stock':''}"><a href="product.html?id=${encodeURIComponent(p.id)}" class="product-image"><img src="${escapeHtml(img)}" alt="${escapeHtml(p.name)}" loading="lazy"><span class="product-category">${escapeHtml(p.category)}</span>${productImages(p).length>1?`<span class="image-count">◈ ${productImages(p).length}</span>`:''}</a><div class="product-info"><a href="product.html?id=${encodeURIComponent(p.id)}"><h3>${escapeHtml(p.name)}</h3></a>${stockMarkup(p)}<div class="product-bottom"><strong>${formatPrice(p.price)}</strong><button class="add-btn" data-add-product="${p.id}" ${out?'disabled':''} onclick="event.preventDefault();addToCart(${Number(p.id)},1)" aria-label="إضافة ${escapeHtml(p.name)} إلى السلة" title="إضافة إلى السلة">${bagIcon()}</button></div></div></article>`}
function renderProducts(list,id){const el=document.getElementById(id);if(!el)return;el.innerHTML=(list||[]).map(productCard).join('')||'<div class="empty-state"><h2>لا توجد منتجات</h2><p>جرّب قسمًا آخر أو غيّر كلمة البحث.</p></div>';initReveal();updateAllCustomerStockUI()}
